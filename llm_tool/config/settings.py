...
class SchedulerConfig:
    """Configuration for intelligent task scheduling in parallel training.

    This config controls when parallel training is offered and how tasks
    are distributed between GPU and CPU workers to minimize total training time.

    Key Features:
    - Automatic thresholds for when to offer parallel training
    - Dynamic GPU/CPU speed ratio tracking
    - Smart task reservation for GPU at end of training
    - CPU cutoff to prevent slow end-of-training scenarios
    """
    ...
    def load(self, config_file: Optional[str] = None):
        ...
        try:
            ...
            if 'paths' in config_data:
                # Always resolve standard subdirectories relative to cwd
                # so the tool works from whichever repo you run it in,
                # regardless of stale absolute paths stored in the config.
                cwd = Path.cwd()
                _standard_subdirs = {
                    'base_dir': cwd,
                    'data_dir': cwd / "data",
                ...
                loaded_paths = {k: Path(v) for k, v in config_data['paths'].items()}
                for key, default_val in _standard_subdirs.items():
                    loaded_paths[key] = default_val
