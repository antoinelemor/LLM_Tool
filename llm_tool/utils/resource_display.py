...
def create_visual_resource_panel(
    ...
    hardware_table.add_column("Component", style="bold cyan", no_wrap=True)
    hardware_table.add_column("Details", style="white")
    hardware_table.add_column("Usage")
    ...
    if gpu.available:
        ...
        if gpu.total_memory_gb > 0:
            ...
            if gpu.available_memory_gb > 0:
                ...
                gpu_usage = Text.from_markup(_create_progress_bar(mem_percent, 10))
    ...
    cpu_usage = Text()
    if cpu.cpu_percent > 0:
        cpu_usage = Text.from_markup(_create_progress_bar(cpu.cpu_percent, 10))
    ...
    mem_usage = Text.from_markup(_create_progress_bar(mem.percent_used, 10))
    ...
    storage_usage = Text.from_markup(_create_progress_bar(storage.percent_used, 10))
    ...
    if show_recommendations:
        ...
        rec_table.add_column("Setting", style="bold green", no_wrap=True)
...
def create_detailed_mode_panel(
    ...
    hw_table.add_column("", style="bold", no_wrap=True)
    hw_table.add_column("")
    ...
    mem_line = Text.from_markup(f"{mem_bar} {_format_gb(resources.memory.available_gb)}/{_format_gb(resources.memory.total_gb)}")
    hw_table.add_row("RAM", mem_line)

    # Right: Recommendations
    rec = resources.get_recommendation()
    rec_table = Table(show_header=False, box=None, padding=(0, 1))
    rec_table.add_column("", style="bold green", no_wrap=True)
