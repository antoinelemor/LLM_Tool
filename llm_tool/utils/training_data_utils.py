...
class TrainingDataSessionManager:
    ...
    def __init__(self, session_id: Optional[str] = None, logs_base_dir: Optional[Path] = None,
        ...
        # NEW: Hyperparameters storage for session manifest generation
        self.hyperparameters: Dict[str, Any] = {}

        # NEW: Categories training results for manifest
        self.categories_results: List[Dict[str, Any]] = []

        # NEW: Session start time for timing
    ...
    def save_model_catalog_csv(self):
        ...
        For hybrid training approach, only includes:
        - Multi-class datasets (for multiclass_keys categories)
        - onevsall_multilabel dataset (for onevsall_keys categories)
        Excludes: multilabel_combined and multilabel datasets
        """
        catalog_rows = []
        model_id = 1

        # Detect training approach from first dataset's metadata
        training_approach_global = None
        for data in self.distribution_data.values():
            training_approach_global = data.get('metadata', {}).get('training_approach')
            if training_approach_global:
                break

        for dataset_name, data in self.distribution_data.items():
            # In hybrid mode, skip datasets that are not used
            # hybrid uses: multiclass files (e.g., manual_sentiment) + onevsall_multilabel
            # hybrid does NOT use: multilabel, multilabel_combined
            if training_approach_global == 'hybrid':
                # Skip the generic multilabel datasets in hybrid mode
                if dataset_name in ('multilabel', 'multilabel_combined'):
                    continue
    ...
    def _write_best_models_performance_section(self, f) -> None:
        """
        Write the best models performance section to the SESSION_SUMMARY.txt file.

        Reads data from all_best_models_final.csv and presents it in a readable format,
        organized by category with key metrics highlighted.
        """
        # Try to find the best models CSV file
        best_models_path = self.session_dir / "all_best_models_final.csv"

        if not best_models_path.exists():
            # Try legacy name
            legacy_paths = list(self.session_dir.glob("*_best_models.csv"))
            if legacy_paths:
                best_models_path = legacy_paths[0]

        if not best_models_path.exists():
            return  # No best models file found, skip this section

        try:
            # Read the CSV, skipping comment lines
            df = pd.read_csv(best_models_path, comment='#')
            ...
            if 'category' in df.columns:
                ...
                # Separate multi-class and binary models
                multiclass_models = []
                binary_models = []

                for _, row in df.iterrows():
                    category = row.get('category', 'unknown')
                    # Detect if binary (one-vs-all) based on category name pattern
                    if 'theme_' in str(category) or category.count('_') > 2:
                        binary_models.append(row)
                    else:
                        multiclass_models.append(row)
                ...
                if multiclass_models:
                    ...
                    for row in multiclass_models:
                        ...
                        # Show per-language metrics if available
                        if 'EN_accuracy' in row and pd.notna(row.get('EN_accuracy')):
                            en_acc = row.get('EN_accuracy', 0)
                            en_f1 = row.get('EN_macro_f1', 0)
                            f.write(f"      EN Performance:  Acc={en_acc:.4f}, F1={en_f1:.4f}\n")
                        if 'FR_accuracy' in row and pd.notna(row.get('FR_accuracy')):
                            fr_acc = row.get('FR_accuracy', 0)
                            fr_f1 = row.get('FR_macro_f1', 0)
                            f.write(f"      FR Performance:  Acc={fr_acc:.4f}, F1={fr_f1:.4f}\n")
                ...
                if binary_models:
                    ...
                    for row in binary_models:
                        category = row.get('category', 'unknown')
                        # Extract parent category (e.g., manual_themes from manual_themes_theme_agriculture)
                        parts = str(category).split('_theme_')
                        if len(parts) > 1:
                            parent = parts[0]
                            label = 'theme_' + parts[1]
                        else:
                            parent = 'other'
                            label = category
                        ...
                        parent_categories[parent].append((label, row))

                    for parent, items in sorted(parent_categories.items()):
                        f.write(f"\n    {parent} ({len(items)} labels)\n")
                        f.write("    " + "─" * 72 + "\n")

                        # Show summary statistics
                        accuracies = [r.get('accuracy', 0) for _, r in items]
                        f1_scores = [r.get('macro_f1', 0) for _, r in items]
                        ...
                        f.write(f"      F1 Range:          {min_f1:.4f} - {max_f1:.4f}\n")

                        # Show top 5 and bottom 3 performers
                        sorted_items = sorted(items, key=lambda x: x[1].get('macro_f1', 0), reverse=True)

                        f.write(f"\n      Top performers:\n")
                        for label, row in sorted_items[:5]:
                ...
                avg_f1 = df['macro_f1'].mean() if 'macro_f1' in df.columns else 0

                f.write(f"    Total Best Models:     {total_models}\n")
            ...
            logger.warning(f"Could not read best models file: {e}")
            # Don't write anything if we can't read the file
    ...
    def finalize(self, training_context: Optional[Dict[str, Any]] = None):
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            ...
            # Best Models Performance Section
            self._write_best_models_performance_section(f)
            ...
            # Check for best models files
            best_models_final = self.session_dir / "all_best_models_final.csv"
            best_detected = self.session_dir / "all_best_detected_models.csv"

            if best_models_final.exists() or best_detected.exists():
                f.write(f"\n  Performance Reports:\n")
                if best_models_final.exists():
                    f.write(f"    - Best Models:       all_best_models_final.csv (final selected models)\n")
                if best_detected.exists():
                    f.write(f"    - All Detected:      all_best_detected_models.csv (training history)\n")
    ...
    def set_hyperparameters(self, hyperparameters: Dict[str, Any]) -> None:
        """
        Store hyperparameters for session manifest generation.
    ...
    def add_category_result(
        ...
        """
        Add a category training result for session manifest.

        Args:
            name: Category name
            huggingface_model: HuggingFace model used
            training_mode: Training mode (multi-class, multi-label, etc.)
            num_labels: Number of labels
            label_names: List of label names
            best_epoch: Best epoch number
            best_macro_f1: Best macro F1 score
            best_accuracy: Best accuracy
            languages: Languages trained on
            training_time_seconds: Training duration
            hyperparameters: Optional hyperparameters override for this category
        """
        self.categories_results.append({
            'name': name,
    ...
    def generate_session_manifest(
        ...
        This creates a single JSON file with all metadata needed for
        experiment reproducibility and analysis.

        Args:
            training_mode: Training mode (normal, parallel, benchmark)
            training_approach: Training approach (multi-class, multi-label, one-vs-all)
            multi_label: Whether multi-label classification was used
            device: Primary device used for training
            status: Final status (completed, failed, completed_with_errors)
        ...
        manifest = {
            ...
            'default_hyperparameters': self.hyperparameters,
            'categories_trained': self.categories_results,
            'files_generated': files_generated or {
                'training_data_dir': str(self.datasets_dir),
                'training_metrics_dir': str(self.training_metrics_dir),
                'session_summary': str(self.training_data_logs_dir / 'SESSION_SUMMARY.txt'),
            },
        }
