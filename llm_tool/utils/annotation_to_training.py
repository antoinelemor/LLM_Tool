...
def _clean_label_value(value: Any) -> Optional[str]:
    ...
    This fixes common LLM output issues like:
    - "null," -> "null"
    - "value, " -> "value"
    - " liberal " -> "liberal"

    Args:
        value: The raw label value from annotation
    ...
    # Remove trailing commas, semicolons, colons (common LLM output artifacts)
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    if isinstance(value, list) and has_valid_value:
                        ...
                        # Keep 'null' as a valid value, clean trailing punctuation
                        ...
                        # For scalars, create single label, clean trailing punctuation
                        cleaned_value = _clean_label_value(value)
                        if cleaned_value:
                            if label_strategy == "key_value":
                                labels = [f"{key}_{cleaned_value}"]
                            else:
                                labels = [cleaned_value]
                        else:
                            labels = []
                    else:
                        ...
                        for other_key, other_value in annotation.items():
                            ...
                            if isinstance(other_value, list):
                                # Keep 'null' as a valid value, clean trailing punctuation
                                clean_values = [_clean_label_value(v) for v in other_value]
                                clean_values = [v for v in clean_values if v is not None and v != '']
                                if label_strategy == "key_value":
                                ...
                                cleaned_other = _clean_label_value(other_value)
                                if cleaned_other:
                                    if label_strategy == "key_value":
                                        labels.append(f"{other_key}_{cleaned_other}")
                                    else:
                                        labels.append(cleaned_other)
            ...
            try:
                ...
                for key in keys_to_use:
                    ...
                    if isinstance(value, dict):
                        ...
                        # Clean the value to remove trailing punctuation
                        cleaned_value = _clean_label_value(value)
                        if cleaned_value:
                            if label_strategy == "key_value":
                                all_labels.append(f"{key}_{cleaned_value}")
                            else:
            ...
            try:
                ...
                if isinstance(value, dict):
                    ...
                    if not cleaned_value:
                        filtered_items.append({
                            'index': idx,
                            'reason': 'empty_cleaned_value',
                            'key': annotation_key
                        })
                        continue
                    if label_strategy == "key_value":
                        label_string = f"{annotation_key}_{cleaned_value}"
                    else:
                        label_string = cleaned_value
