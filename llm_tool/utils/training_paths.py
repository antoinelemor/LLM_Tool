...
def get_current_session_dir() -> Optional[Path]:
    """
    Return the currently active session directory, if one has been set.

    This is useful for code that needs to write files within the session
    but doesn't have the session_id available.

    Returns:
        The active session directory path, or None if not set.
    """
    return _TRAINING_SESSION_DIR
...
def resolve_metrics_base_dir(configured: Optional[Union[str, Path]] = None) -> Path:
    ...
    if configured:
        ...
        if configured_path != Path("logs/training_arena") or _TRAINING_SESSION_DIR is None:
            return configured_path

    # Second priority: active session directory
    if _TRAINING_SESSION_DIR is not None:
        return _TRAINING_SESSION_DIR

    # Fallback: base directory
    return get_training_logs_base()
