...
class TrainingSummaryGenerator:
    ...
    def __init__(self, session_id: str, session_dir: Path):
        ...
        # Note: No versioned copies needed since session_id is already in directory path
    ...
    def generate_comprehensive_summaries(self) -> Tuple[Path, Path]:
        ...
        try:
            ...
            logger.info(f"  Files: training_summary_final.csv/jsonl, all_best_detected_models.csv")
    ...
    def _prepare_label_legend(self) -> str:
        ...
        For multi-label data (where labels are tuples/combinations), this extracts
        and displays the unique individual labels with their total occurrence counts,
        rather than showing every possible combination.
        """
        legend_lines = []
        ...
        if "dataset_info" in self.session_data and "summary" in self.session_data["dataset_info"]:
            ...
            if label_dist:
                # Check if this is multi-label data (labels are tuples)
                is_multi_label = any(isinstance(label, (tuple, list, frozenset)) for label in label_dist.keys())

                if is_multi_label:
                    # Extract individual labels and count their occurrences
                    individual_label_counts: dict = {}
                    total_samples = 0
                    num_combinations = len(label_dist)

                    for label_combo, count in label_dist.items():
                        total_samples += count
                        if isinstance(label_combo, (tuple, list, frozenset)):
                            for individual_label in label_combo:
                                if individual_label:  # Skip empty
                                    individual_label_counts[individual_label] = individual_label_counts.get(individual_label, 0) + count
                        elif label_combo:  # Single label
                            individual_label_counts[label_combo] = individual_label_counts.get(label_combo, 0) + count

                    # Format legend for multi-label
                    legend_lines.append(f"# ═══════════════════════════════════════════════════════════════")
                    legend_lines.append(f"# LABEL SUMMARY (Multi-Label Classification)")
                    legend_lines.append(f"# ═══════════════════════════════════════════════════════════════")
                    legend_lines.append(f"# Total samples: {total_samples}")
                    legend_lines.append(f"# Unique label combinations: {num_combinations}")
                    legend_lines.append(f"# Unique individual labels: {len(individual_label_counts)}")
                    legend_lines.append(f"#")
                    legend_lines.append(f"# INDIVIDUAL LABEL COUNTS:")
                    legend_lines.append(f"# (Each label can appear in multiple samples due to multi-labeling)")
                    legend_lines.append(f"# ───────────────────────────────────────────────────────────────")

                    # Sort by count descending
                    sorted_labels = sorted(individual_label_counts.items(), key=lambda x: -x[1])
                    for idx, (label, count) in enumerate(sorted_labels, 1):
                        # Clean up label name for display
                        display_label = str(label)
                        if display_label.startswith("consensus_themes_large_"):
                            display_label = display_label.replace("consensus_themes_large_", "")
                        pct = 100 * count / total_samples if total_samples > 0 else 0
                        legend_lines.append(f"# {idx:3d}. {display_label:45s} {count:5d} ({pct:5.1f}%)")
                    ...
                    sorted_entries = sorted(formatted_entries, key=lambda item: item[0])
                    total_samples = sum(count for _, count in sorted_entries)

                    if len(sorted_entries) > 20:
                        # Concise format for large label sets to avoid bloated legends
                        mapping = ", ".join(
                            f"{idx}={label_text}" for idx, (label_text, _) in enumerate(sorted_entries)
                        )
                        legend_lines.append(f"# CLASS_LEGEND: {mapping}")
                        legend_lines.append(f"# Total unique labels: {len(sorted_entries)}, Total samples: {total_samples}")
                    else:
                        # Detailed format for small label sets
                        legend_lines.append("# LABEL LEGEND:")
                        for idx, (label_text, count) in enumerate(sorted_entries, 1):
    ...
    def generate_best_models_final_csv(self) -> Path:
        """
        Generate all_best_detected_models.csv with all best models detected during training.

        This file contains every best model checkpoint detected at each epoch,
        providing a complete record of training progress and model selection.
        """
        best_models_path = self.session_dir / "all_best_detected_models.csv"
        ...
        try:
            if "_best_models_df" in self.session_data and not self.session_data["_best_models_df"].empty:
                ...
                logger.info(f"Generated all_best_detected_models.csv with {len(best_df)} detected models")
            else:
                ...
                logger.warning("No best models found, created empty all_best_detected_models.csv")

            return best_models_path

        except Exception as e:
            logger.error(f"Failed to generate all_best_detected_models.csv: {e}")
            # Create minimal file
