...
def compare_model_results(
    ...
    for model_id, result in results.items():
        ...
        f1_micro = result.get('f1_micro', result.get('micro_f1', result.get('best_f1_micro', 0)))
        ...
        # Calculate combined metric (matching epoch selection logic in bert_base.py)
        ...
        if use_sophisticated_ranking:
            ...
            if is_binary and f1_macro > 0:
                ...
                # This ensures model doesn't ignore rare classes (macro) while maintaining
                # good overall performance (micro)
                if f1_micro > 0:
                    combined_score = 0.6 * f1_macro + 0.4 * f1_micro
                else:
                    # Fallback if micro not available
            ...
            if is_binary:
                ...
                if f1_micro > 0:
                    explanation_parts.append(
                        f"Combined: 60%×F1ₘₐ({f1_macro:.3f}) + 40%×F1ₘᵢ({f1_micro:.3f})"
                    )
                else:
                    explanation_parts.append(f"F1 Macro: {f1_macro:.3f} (micro N/A)")
    ...
    # Sophisticated ranking: Combined score (primary), Accuracy (secondary)
    # NOTE: No speed/time criteria - selection based on quality only
    df = df.sort_values(
...
def consolidate_session_csvs(session_dir: Path, session_id: str) -> Dict[str, Path]:
    ...
    - all_best_models_final.csv: ONLY the final best selected models, organized for readability
    ...
    if best_csvs:
        ...
        if all_best_data:
            ...
            if 'combined_score' not in consolidated_df.columns:
                ...
                elif 'macro_f1' in consolidated_df.columns and 'micro_f1' in consolidated_df.columns:
                    # Multi-class/Multi-label: 60% macro + 40% micro
                    consolidated_df['combined_score'] = (
                        0.6 * consolidated_df['macro_f1'].fillna(0) +
                        0.4 * consolidated_df['micro_f1'].fillna(0)
                    )
                elif 'macro_f1' in consolidated_df.columns:
                    # Multi-class fallback (no micro available): use macro F1
            ...
            best_output = output_dir / "all_best_models_final.csv"

            # Reorganize columns for better readability
            # Priority order: identification → key metrics → detailed metrics → paths
            priority_cols = [
                # Identification
                'mode', 'category', 'language', 'model_type',
                # Training info
                'epoch', 'training_phase',
                # Key metrics (most important)
                'combined_score', 'macro_f1', 'accuracy',
                # Loss metrics
                'train_loss', 'val_loss',
                # Timestamp
                'timestamp',
            ...
            existing_cols = list(consolidated_df.columns)

            # Build ordered column list: priority cols first, then remaining
            ordered_cols = [col for col in priority_cols if col in existing_cols]
            remaining_cols = [col for col in existing_cols if col not in ordered_cols]

            # Sort remaining columns: per-class metrics, then language-specific, then paths
            class_metrics = sorted([c for c in remaining_cols if c.startswith(('precision_', 'recall_', 'f1_', 'support_')) and not c.startswith(('EN_', 'FR_'))])
            en_metrics = sorted([c for c in remaining_cols if c.startswith('EN_')])
            fr_metrics = sorted([c for c in remaining_cols if c.startswith('FR_')])
            path_cols = [c for c in remaining_cols if 'path' in c.lower()]
            other_cols = [c for c in remaining_cols if c not in class_metrics + en_metrics + fr_metrics + path_cols]

            final_col_order = ordered_cols + other_cols + class_metrics + en_metrics + fr_metrics + path_cols
...
def select_representative_benchmark_items(
    bundle_metadata: Dict[str, Any],
    data: pd.DataFrame,
    annotation_column: str = 'labels',
    max_multiclass: int = 1,
    max_onevsall_per_category: int = 2
) -> Dict[str, Any]:
    """
    Automatically select representative items for benchmark based on hybrid mode strategy.

    For multi-class categories: Select the one with most samples (most reliable metrics)
    For one-vs-all categories: Select 1-2 representative key+value pairs:
        - 1 balanced (lowest imbalance ratio)
        - 1 imbalanced (highest imbalance ratio)

    This ensures the benchmark tests both multi-class and one-vs-all strategies
    without training ALL values (which would be too slow).

    Args:
        bundle_metadata: The training bundle's metadata containing:
            - multiclass_keys: List of keys using multi-class strategy
            - onevsall_keys: List of keys using one-vs-all strategy
            - all_keys_values: Dict mapping each key to its list of values
            - value_counts_by_key: Dict with sample counts per value
        data: DataFrame with annotations
        annotation_column: Column containing annotations
        max_multiclass: Maximum number of multi-class categories to select
        max_onevsall_per_category: Maximum key+value pairs per one-vs-all category

    Returns:
        Dict containing:
            - selected_items: List of selected benchmark items (category names or key_value pairs)
            - multiclass_items: List of selected multi-class categories
            - onevsall_items: List of selected one-vs-all key_value pairs
            - selection_rationale: Dict explaining why each item was selected
            - total_models: Total number of models that will be trained
            - strategy_breakdown: Dict showing models per strategy
    """
    import json

    multiclass_keys = bundle_metadata.get('multiclass_keys', [])
    onevsall_keys = bundle_metadata.get('onevsall_keys', [])
    all_keys_values = bundle_metadata.get('all_keys_values', {})
    value_counts_by_key = bundle_metadata.get('value_counts_by_key', {})
    ...
    # Select the category with most samples for reliable metrics
    # ============================================================
    if multiclass_keys:
        # Calculate total samples per multi-class category
        mc_sample_counts = {}
        for key in multiclass_keys:
            if key in value_counts_by_key:
                mc_sample_counts[key] = sum(value_counts_by_key[key].values())
            else:
                # Count from data if not in metadata
                mc_sample_counts[key] = 0
                for _, row in data.iterrows():
                    ann = row.get(annotation_column, {})
                    if isinstance(ann, str):
                        try:
                            ann = json.loads(ann)
                        except:
                            continue
                    if isinstance(ann, dict) and key in ann and ann[key]:
                        mc_sample_counts[key] += 1

        # Sort by sample count and select top N
        sorted_mc = sorted(mc_sample_counts.items(), key=lambda x: x[1], reverse=True)

        for key, count in sorted_mc[:max_multiclass]:
            multiclass_items.append(key)
            selected_items.append(key)
            num_classes = len(all_keys_values.get(key, []))
            selection_rationale[key] = {
                'strategy': 'multi-class',
                'reason': f'Most samples ({count}), {num_classes} classes',
                'samples': count,
                'num_classes': num_classes,
                'models': 1  # Multi-class = 1 model
            }

    # ============================================================
    # ONE-VS-ALL SELECTION
    # For each one-vs-all category, select:
    # - 1 balanced value (lowest imbalance ratio)
    # - 1 imbalanced value (highest imbalance ratio)
    # ============================================================
    if onevsall_keys:
        for key in onevsall_keys:
            values = all_keys_values.get(key, [])
            value_counts = value_counts_by_key.get(key, {})

            if not values or not value_counts:
                continue

            # Calculate imbalance ratio for each value
            # Ratio = (negative samples) / (positive samples)
            # Higher ratio = more imbalanced (harder to learn)
            total_samples = sum(value_counts.values())
            value_ratios = []

            for value in values:
                positive_count = value_counts.get(value, 0)
                if positive_count > 0:
            ...
            # Sort by ratio
            sorted_by_ratio = sorted(value_ratios, key=lambda x: x['ratio'])

            selected_for_key = []

            # Select most balanced (lowest ratio)
            if sorted_by_ratio:
                balanced = sorted_by_ratio[0]
                key_value_name = balanced['key_value']
                selected_for_key.append(key_value_name)
                onevsall_items.append({
                    'category': key,
                    'value': balanced['value'],
                    'key_value': key_value_name,
                    'type': 'balanced'
                })
                selected_items.append(key_value_name)
                selection_rationale[key_value_name] = {
                    'strategy': 'one-vs-all',
                    'type': 'balanced',
                    'reason': f"Most balanced in '{key}' (ratio {balanced['ratio']:.1f}:1)",
                    'positive_samples': balanced['positive'],
                    'negative_samples': balanced['negative'],
                    'ratio': balanced['ratio'],
                    'models': 1
                }

            # Select most imbalanced (highest ratio) if different and we want 2
            if max_onevsall_per_category >= 2 and len(sorted_by_ratio) > 1:
                imbalanced = sorted_by_ratio[-1]
                # Only add if significantly different from balanced
                if imbalanced['ratio'] > sorted_by_ratio[0]['ratio'] * 1.5:
                    key_value_name = imbalanced['key_value']
                    selected_for_key.append(key_value_name)
                    onevsall_items.append({
                        'category': key,
                        'value': imbalanced['value'],
                        'key_value': key_value_name,
                        'type': 'imbalanced'
                    })
                    selected_items.append(key_value_name)
                    selection_rationale[key_value_name] = {
                        'strategy': 'one-vs-all',
                        'type': 'imbalanced',
                        'reason': f"Most imbalanced in '{key}' (ratio {imbalanced['ratio']:.1f}:1)",
                        'positive_samples': imbalanced['positive'],
                        'negative_samples': imbalanced['negative'],
                        'ratio': imbalanced['ratio'],
                        'models': 1
                    }

    # Calculate totals
    total_models = len(multiclass_items) + len(onevsall_items)

    strategy_breakdown = {
        'multi-class': {
            'categories': multiclass_items,
            'models': len(multiclass_items)
        },
        'one-vs-all': {
            'items': onevsall_items,
            'models': len(onevsall_items),
            'original_total': sum(len(all_keys_values.get(k, [])) for k in onevsall_keys)
        }
    }

    return {
        'selected_items': selected_items,
        'multiclass_items': multiclass_items,
        'onevsall_items': onevsall_items,
