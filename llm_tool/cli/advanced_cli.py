...
class AdvancedCLI:
    ...
    def get_main_menu_choice(self) -> str:
        ...
        if HAS_RICH and self.console:
            ...
            options = [
                ("1", "🎤 Data Preparation Studio - Audio Extraction (YouTube/TikTok/Local) → Whisper Transcription (75+ Languages) → NLP Pipeline"),
                ("2", "🎨 The Annotator - Zero-Shot LLM Annotation (OpenAI/Ollama) → Label Studio/Doccano Export"),
                ("3", "🏭 The Annotator Factory - LLM Annotations → Training Data → Fine-Tuned BERT Models"),
                ("4", "🎮 Training Arena - Train 50+ Models (BERT/RoBERTa/DeBERTa) with Multi-Label & Benchmarking"),
                ("5", "🤖 BERT Annotation Studio - High-Throughput Inference (Parallel GPU/CPU, 100+ Languages)"),
                ("6", "🔍 Validation Lab - Quality Scoring, Stratified Sampling, Inter-Annotator Agreement [⚠️ IN DEVELOPMENT]"),
                ("7", "📂 Resume Center - Manage Sessions & Configurations"),
            ...
            print("Main Menu")
            print("="*50)
            print("1. Data Preparation Studio - Audio Extraction (YouTube/TikTok/Local) → Transcription → NLP Pipeline")
            print("2. The Annotator - Zero-Shot LLM Annotation (OpenAI/Ollama) → Export")
            print("3. The Annotator Factory - LLM Annotations → Training Data → BERT Models")
            print("4. Training Arena - Train 50+ Models (Multi-Label & Benchmarking)")
            print("5. BERT Annotation Studio - High-Throughput Inference (Parallel GPU/CPU)")
            print("6. Validation Lab - Quality Scoring & Sampling [⚠️ IN DEVELOPMENT]")
            print("7. Resume Center - Manage Sessions & Configurations")
    ...
    def _resume_mode_roots(self) -> Dict[str, Path]:
        ...
        return {
            ...
            "data_preparation": Path("logs") / "data_preparation",
        }
    ...
    def resume_center(self) -> None:
        ...
        if mode == "data_preparation":
            self._dp_resume_session(chosen)
        elif mode == "annotator":
    ...
    def quick_start_wizard(self):
        ...
        self._display_section_header(
            ...
            mode_info={
                ...
                'capabilities': ['2 LLM Providers (OpenAI/Ollama)', '100+ Languages', 'Multi-Label Classification', 'Reinforcement Learning'],
    ...
    def _execute_mode2_from_metadata(self, metadata: dict, action_mode: str, metadata_file: Path):
        ...
        if run_annotation:
            ...
            pipeline_config = {
                ...
                'create_annotated_subset': proc_config.get('create_annotated_subset', True),
...
Format your response as JSON with keys: topic, sentiment, entities, summary"""
    ...
    def _dp_create_session_directories(self, session_id: str) -> Dict[str, Path]:
        """Create organised directory structure for a data preparation session."""
        base_dir = Path("logs") / "data_preparation" / session_id
        dirs: Dict[str, Path] = {
            "base": base_dir,
            "session_root": base_dir,
            "audio": base_dir / "audio",
            "transcripts": base_dir / "transcripts",
            "tokenization": base_dir / "tokenization",
    ...
    def _dp_init_session_logger(self, session_dirs: Dict[str, Path]) -> logging.Logger:
        """Create a dedicated file logger for a data preparation session."""
        session_log_file = session_dirs["base"] / "session.log"
        logger = logging.getLogger(f"data_preparation.{session_dirs['base'].name}")
        logger.setLevel(logging.DEBUG)
        logger.propagate = True  # Also propagate to root (global log file)
        # Avoid duplicate handlers on repeated calls
        if not logger.handlers:
            fh = logging.FileHandler(session_log_file, mode="a", encoding="utf-8")
    ...
    def _dp_persist_metadata(self, session_dirs: Dict[str, Path], metadata: Dict[str, Any]) -> None:
        """Write session metadata to disk."""
        metadata_path = session_dirs["metadata"] / "session_metadata.json"
        metadata_path.parent.mkdir(parents=True, exist_ok=True)
        with metadata_path.open("w", encoding="utf-8") as f:
            json.dump(metadata, f, indent=2, ensure_ascii=False, default=str)

    def _dp_update_resume(
        self,
        ...
        """Create or update resume.json for Resume Center integration."""
        from ..utils.session_summary import SessionSummary, merge_summary

        summary = SessionSummary(
            mode="data_preparation",
            session_id=session_id,
            session_name=session_name,
            status=status,
        )
        if step_key:
            summary.record_step(key=step_key, name=step_name, number=step_no)
        if extra:
            summary.extra.update(extra)

        resume_path = session_dirs["base"] / "resume.json"
        merge_summary(resume_path, summary)

    def _get_transcribe_cli(self, session_dirs: Optional[Dict[str, Path]] = None):
        """Instantiate TranscribeCLI with optional output routing to session dirs."""
        from ..transcription.cli import TranscribeCLI

        cli = TranscribeCLI()
        # Route outputs to session directories when a session is active
        if session_dirs:
            cli.config.audio_dir = session_dirs["audio"]
            cli.config.transcripts_dir = session_dirs["transcripts"]
        return cli

    def data_preparation_studio(self):
        """
        Data Preparation Studio - Audio Extraction, Transcription & NLP Pipeline.

        Features:
        - YouTube audio extraction (single video, channel, playlist)
        - TikTok audio extraction (single video, user profile)
        - Local audio/video file processing
        - OpenAI Whisper transcription (tiny to large-v3, 75+ languages)
        - Speaker diarization (pyannote 3.1)
        - SOTA sentence segmentation & NLP tokenization
        - Full pipeline with checkpoint resume
        - Multiple output formats (TXT, CSV, JSON, SRT, WebVTT)
        - Session-based logging with metadata persistence
        """
        # Check transcription module availability first
        try:
            from ..transcription.cli import TranscribeCLI
        except ImportError as e:
            self.logger.error(f"Transcription module not available: {e}")
            if HAS_RICH and self.console:
                self.console.print(f"\n[bold red]Transcription module not available:[/bold red] {e}")
                self.console.print("[yellow]Install transcription dependencies with: pip install llm-tool\\[transcription][/yellow]\n")
            else:
                print(f"\nTranscription module not available: {e}")
                print("Install transcription dependencies with: pip install llm-tool[transcription]\n")
            return
        ...
        # Display mode-specific banner
        self._display_mode_banner('data_preparation')

        # Display personalized mode info
        self._display_section_header(
            "🎤 Data Preparation Studio - Audio Extraction, Transcription & NLP Pipeline",
            "Extract audio from any source, transcribe with Whisper, and process text with SOTA NLP tools",
            mode_info={
                'workflow': 'Audio Source (YouTube/TikTok/Local) → Extract → Whisper Transcribe → Text Processing → Export',
                'capabilities': ['YouTube/TikTok/Local Extraction', 'Whisper (tiny→large-v3)', 'Speaker Diarization', '75+ Languages', 'SOTA Sentence Segmentation', 'NLP Tokenization'],
                'input': 'YouTube/TikTok URLs, local audio/video files (MP3, WAV, M4A, FLAC, MP4, MKV, ...)',
                'output': 'Transcriptions (TXT/CSV/JSON/SRT/WebVTT), tokenized datasets (CSV/JSON)',
                'best_for': 'Preparing audio/video data for annotation pipelines, creating text corpora from multimedia'
            }
        )

        self.logger.info("Entered Data Preparation Studio")
        ...
        if HAS_RICH and self.console:
            ...
            resume_records = self._fetch_resume_records("data_preparation", limit=10)
            ...
            workflows = [
                ("1", "🎬 Extract Audio from YouTube (Single Video, Channel, or Playlist)"),
                ("2", "📱 Extract Audio from TikTok (Single Video or User Profile)"),
                ("3", "📂 Process Local Audio/Video Files (Single File or Directory)"),
                ("4", "🎙️  Transcribe Audio Files (Whisper, Diarization, Multi-Format Export)"),
                ("5", "🔄 Full Pipeline (Extract → Transcribe → Export) [Resume Supported]"),
                ("6", "📝 Text Tokenization (Documents → SOTA NLP Segmentation → CSV/JSON)"),
                ("7", "⚙️  Settings (Whisper Model, Device, Cookies, HuggingFace Token)"),
            ]

            if resume_records:
                workflows.append(("8", f"📂 Resume Previous Session ({len(resume_records)} available)"))

            workflows.append(("0", "⬅️  Back to main menu"))
            ...
            panel = Panel(
                workflow_table,
                title="[bold]🎤 Data Preparation Studio[/bold]",
                subtitle=f"[dim]{suggestions}[/dim]" if suggestions else None,
                border_style="cyan"
            )
            ...
            print("\n=== Data Preparation Studio ===")
            print("Audio Extraction, Transcription & NLP Pipeline\n")
            print("1. Extract Audio from YouTube")
            print("2. Extract Audio from TikTok")
            print("3. Process Local Audio/Video Files")
            print("4. Transcribe Audio Files")
            print("5. Full Pipeline (Extract → Transcribe → Export)")
            print("6. Text Tokenization")
            print("7. Settings")

            resume_records = self._fetch_resume_records("data_preparation", limit=10)
            if resume_records:
                print(f"8. Resume Previous Session ({len(resume_records)} available)")
        ...
        if workflow == "0":
            return
        elif workflow == "8" and resume_records:
            self._dp_show_resume_menu(resume_records)
            return

        # Settings doesn't need a session
        if workflow == "7":
            self._dp_settings()
            return
        ...
        workflow_labels = {
            "1": "youtube_extraction", "2": "tiktok_extraction",
            "3": "local_processing", "4": "transcription",
            "5": "full_pipeline", "6": "tokenization",
        }
        default_name = workflow_labels.get(workflow, "data_preparation")
        ...
        if HAS_RICH and self.console:
            ...
            self.console.print("[bold]Why session names matter:[/bold]")
            self.console.print("  • [green]Organization:[/green] Easily identify data preparation projects")
            self.console.print("  • [green]Traceability:[/green] All audio, transcripts, and exports are stored together")
            self.console.print("  • [green]Resume:[/green] Interrupted pipelines can be resumed from checkpoint")
            self.console.print("  • [green]Audit trail:[/green] Timestamp ensures uniqueness\n")

            self.console.print("[dim]Format: {session_name}_{yyyymmdd_hhmmss}[/dim]")
            self.console.print(f"[dim]Example: {default_name}_20260131_143022[/dim]\n")

            user_session_name = Prompt.ask(
                "[bold yellow]Enter a descriptive name for this session[/bold yellow]",
                default=default_name
            ).strip()
        else:
            print("\n--- Session Name ---")
            print(f"Default: {default_name}")
            user_session_name = input(f"Session name [{default_name}]: ").strip() or default_name

        # Sanitize
        user_session_name = user_session_name.replace(' ', '_').replace('/', '_').replace('\\', '_')
        user_session_name = ''.join(c for c in user_session_name if c.isalnum() or c in ['_', '-'])
        if not user_session_name:
            user_session_name = default_name
        ...
        # Create session directories
        session_dirs = self._dp_create_session_directories(session_id)
        session_logger = self._dp_init_session_logger(session_dirs)

        if HAS_RICH and self.console:
            self.console.print(f"\n[bold green]✓ Session ID:[/bold green] [cyan]{session_id}[/cyan]")
            self.console.print(f"[dim]Session directory: {session_dirs['base']}[/dim]\n")

        session_logger.info(f"Session created: {session_id}")
        session_logger.info(f"Workflow selected: {workflow} ({workflow_labels.get(workflow, 'unknown')})")
        self.logger.info(f"Data Preparation session created: {session_id}")
        ...
        metadata = {
            ...
            "mode": "data_preparation",
            "workflow": workflow_labels.get(workflow, "unknown"),
            "created_at": dt.now().strftime("%Y-%m-%dT%H:%M:%S"),
            "updated_at": dt.now().strftime("%Y-%m-%dT%H:%M:%S"),
            "status": "active",
            "directories": {k: str(v) for k, v in session_dirs.items()},
            "steps": {},
        }
        self._dp_persist_metadata(session_dirs, metadata)

        # Initialize resume.json
        self._dp_update_resume(
            session_dirs, session_id, user_session_name,
            status="active",
            step_key="init", step_name="Session initialized", step_no=0,
            extra={"workflow": workflow_labels.get(workflow, "unknown"), "mode_label": "DATA_PREPARATION"},
        )

        # ── Dispatch to workflow ──────────────────────────────────────
        dispatch = {
            "1": ("youtube_extraction", "YouTube Audio Extraction", self._dp_youtube_extraction),
            "2": ("tiktok_extraction", "TikTok Audio Extraction", self._dp_tiktok_extraction),
            "3": ("local_processing", "Local File Processing", self._dp_local_processing),
            "4": ("transcription", "Audio Transcription", self._dp_transcription),
            "5": ("full_pipeline", "Full Pipeline", self._dp_full_pipeline),
            "6": ("tokenization", "Text Tokenization", self._dp_tokenization),
        }

        step_key, step_name, handler = dispatch[workflow]

        self._dp_update_resume(
            session_dirs, session_id, user_session_name,
            status="active",
            step_key=step_key, step_name=step_name, step_no=1,
        )

        session_logger.info(f"Starting workflow: {step_name}")
        metadata["steps"][step_key] = {"status": "in_progress", "started_at": dt.now().strftime("%Y-%m-%dT%H:%M:%S")}
        self._dp_persist_metadata(session_dirs, metadata)

        try:
            handler(session_dirs, session_logger)

            # Mark completed
            metadata["status"] = "completed"
            metadata["updated_at"] = dt.now().strftime("%Y-%m-%dT%H:%M:%S")
            metadata["steps"][step_key]["status"] = "completed"
            metadata["steps"][step_key]["completed_at"] = dt.now().strftime("%Y-%m-%dT%H:%M:%S")
            self._dp_persist_metadata(session_dirs, metadata)

            self._dp_update_resume(
                session_dirs, session_id, user_session_name,
                status="completed",
                step_key=step_key, step_name=f"{step_name} (done)", step_no=1,
            )

            session_logger.info(f"Workflow completed: {step_name}")
            self.logger.info(f"Data Preparation session completed: {session_id}")

            if HAS_RICH and self.console:
                self.console.print(f"\n[bold green]✓ Session completed:[/bold green] [cyan]{session_id}[/cyan]")
                self.console.print(f"[dim]All outputs saved in: {session_dirs['base']}[/dim]\n")

        except KeyboardInterrupt:
            metadata["status"] = "interrupted"
            metadata["updated_at"] = dt.now().strftime("%Y-%m-%dT%H:%M:%S")
            metadata["steps"][step_key]["status"] = "interrupted"
            self._dp_persist_metadata(session_dirs, metadata)

            self._dp_update_resume(
                session_dirs, session_id, user_session_name,
                status="interrupted",
                step_key=step_key, step_name=f"{step_name} (interrupted)", step_no=1,
            )

            session_logger.warning(f"Workflow interrupted: {step_name}")
            if HAS_RICH and self.console:
                self.console.print(f"\n[yellow]Session interrupted: {session_id}[/yellow]")
                self.console.print(f"[dim]Partial outputs saved in: {session_dirs['base']}[/dim]")
            else:
            ...
            metadata["updated_at"] = dt.now().strftime("%Y-%m-%dT%H:%M:%S")
            metadata["steps"][step_key]["status"] = "failed"
            metadata["steps"][step_key]["error"] = str(e)
            self._dp_persist_metadata(session_dirs, metadata)

            self._dp_update_resume(
                session_dirs, session_id, user_session_name,
                status="failed",
                step_key=step_key, step_name=f"{step_name} (failed)", step_no=1,
                extra={"status_note": str(e)},
            )

            session_logger.error(f"Workflow failed: {step_name} - {e}", exc_info=True)
            self.logger.error(f"Data Preparation session failed: {session_id} - {e}", exc_info=True)

            if HAS_RICH and self.console:
                self.console.print(f"\n[bold red]Error:[/bold red] {e}")
                self.console.print(f"[dim]Session logs: {session_dirs['base'] / 'session.log'}[/dim]")
            else:
                print(f"\nError: {e}")

    def _dp_show_resume_menu(self, records):
        """Display resumable data preparation sessions and handle selection."""
        if HAS_RICH and self.console:
            from rich.table import Table
            table = Table(title="📂 Data Preparation Sessions", border_style="cyan")
            table.add_column("#", justify="right", style="cyan", width=4)
            table.add_column("Session", style="white")
            table.add_column("Workflow", style="yellow", width=18)
            ...
            for idx, record in enumerate(records, 1):
                ...
                table.add_row(
                    str(idx),
                    s.session_name or s.session_id,
                    s.extra.get("workflow", "-"),
                    s.status,
                    (s.updated_at or "").replace("T", " "),
                    s.last_step_name or s.last_step_key or "-",
                )
            ...
            for idx, record in enumerate(records, 1):
                s = record.summary
                print(f"  {idx}. {s.session_id} [{s.status}] - {s.extra.get('workflow', '-')}")
        ...
        chosen = records[selection - 1]
        self._dp_resume_session(chosen)

    def _dp_resume_session(self, record):
        """Resume a data preparation session from its saved state."""
        session_dir = record.directory
        summary = record.summary
        session_id = summary.session_id

        self.logger.info(f"Resuming data preparation session: {session_id}")

        # Re-create session dirs dict from existing directory
        session_dirs = {
            "base": session_dir,
            "session_root": session_dir,
            "audio": session_dir / "audio",
            "transcripts": session_dir / "transcripts",
            "tokenization": session_dir / "tokenization",
        ...
        session_logger = self._dp_init_session_logger(session_dirs)
        session_logger.info(f"Session resumed: {session_id}")

        # Update resume status
        self._dp_update_resume(
            session_dirs, session_id, summary.session_name,
            status="active",
            step_key="resumed", step_name="Session resumed",
        )

        if HAS_RICH and self.console:
            self.console.print(f"\n[bold green]✓ Resuming session:[/bold green] [cyan]{session_id}[/cyan]")
            self.console.print(f"[dim]Session directory: {session_dir}[/dim]\n")

        # Determine workflow from metadata or extra
        workflow = summary.extra.get("workflow", "full_pipeline")
        workflow_map = {
            "youtube_extraction": ("YouTube Audio Extraction", self._dp_youtube_extraction),
            "tiktok_extraction": ("TikTok Audio Extraction", self._dp_tiktok_extraction),
            "local_processing": ("Local File Processing", self._dp_local_processing),
            "transcription": ("Audio Transcription", self._dp_transcription),
            "full_pipeline": ("Full Pipeline", self._dp_full_pipeline),
            "tokenization": ("Text Tokenization", self._dp_tokenization),
        }

        step_name, handler = workflow_map.get(workflow, ("Full Pipeline", self._dp_full_pipeline))
        ...
        metadata["updated_at"] = dt.now().strftime("%Y-%m-%dT%H:%M:%S")
        if "resume_log" not in metadata:
            metadata["resume_log"] = []
        metadata["resume_log"].append({"resumed_at": dt.now().strftime("%Y-%m-%dT%H:%M:%S")})
        self._dp_persist_metadata(session_dirs, metadata)

        session_logger.info(f"Relaunching workflow: {step_name}")
        ...
        try:
            ...
            self._dp_update_resume(
                session_dirs, session_id, summary.session_name,
                status="completed",
                step_key=workflow, step_name=f"{step_name} (done)",
            )

            session_logger.info(f"Workflow completed on resume: {step_name}")
            if HAS_RICH and self.console:
                self.console.print(f"\n[bold green]✓ Session completed:[/bold green] [cyan]{session_id}[/cyan]")
                self.console.print(f"[dim]All outputs saved in: {session_dirs['base']}[/dim]\n")
            ...
            self._dp_update_resume(
                session_dirs, session_id, summary.session_name,
                status="interrupted", step_key=workflow, step_name=f"{step_name} (interrupted)",
            )
            session_logger.warning(f"Workflow interrupted on resume: {step_name}")
            if HAS_RICH and self.console:
                self.console.print(f"\n[yellow]Session interrupted: {session_id}[/yellow]")
        except Exception as e:
            metadata["status"] = "failed"
            ...
            self._dp_update_resume(
                session_dirs, session_id, summary.session_name,
                status="failed", step_key=workflow, step_name=f"{step_name} (failed)",
                extra={"status_note": str(e)},
            )
            session_logger.error(f"Workflow failed on resume: {step_name} - {e}", exc_info=True)
            if HAS_RICH and self.console:
                self.console.print(f"\n[bold red]Error:[/bold red] {e}")
                self.console.print(f"[dim]Session logs: {session_dirs['base'] / 'session.log'}[/dim]")

    def _dp_youtube_extraction(self, session_dirs: Dict[str, Path], session_logger: logging.Logger):
        """YouTube audio extraction workflow with session logging."""
        session_logger.info("Starting YouTube audio extraction")
        cli = self._get_transcribe_cli(session_dirs)
        cli._youtube_workflow()
        session_logger.info(f"YouTube extraction complete. Audio files in: {session_dirs['audio']}")

    def _dp_tiktok_extraction(self, session_dirs: Dict[str, Path], session_logger: logging.Logger):
        """TikTok audio extraction workflow with session logging."""
        session_logger.info("Starting TikTok audio extraction")
        cli = self._get_transcribe_cli(session_dirs)
        cli._tiktok_workflow()
        session_logger.info(f"TikTok extraction complete. Audio files in: {session_dirs['audio']}")

    def _dp_local_processing(self, session_dirs: Dict[str, Path], session_logger: logging.Logger):
        """Local audio/video file processing workflow with session logging."""
        session_logger.info("Starting local file processing")
        cli = self._get_transcribe_cli(session_dirs)
        cli._local_workflow()
        session_logger.info(f"Local processing complete. Audio files in: {session_dirs['audio']}")

    def _dp_transcription(self, session_dirs: Dict[str, Path], session_logger: logging.Logger):
        """Audio transcription workflow with session logging."""
        session_logger.info("Starting audio transcription (Whisper)")
        cli = self._get_transcribe_cli(session_dirs)
        session_logger.info(f"Whisper model: {cli.config.whisper_model}, Device: {cli.config.device}")
        cli._transcribe_workflow()
        session_logger.info(f"Transcription complete. Outputs in: {session_dirs['transcripts']}")

    def _dp_full_pipeline(self, session_dirs: Dict[str, Path], session_logger: logging.Logger):
        """Full pipeline workflow with session logging and checkpoint resume."""
        session_logger.info("Starting full pipeline (Extract → Transcribe → Export)")
        cli = self._get_transcribe_cli(session_dirs)
        session_logger.info(f"Whisper model: {cli.config.whisper_model}, Device: {cli.config.device}")
        cli._full_pipeline_workflow()
        session_logger.info(f"Full pipeline complete. Outputs in: {session_dirs['base']}")

    def _dp_tokenization(self, session_dirs: Dict[str, Path], session_logger: logging.Logger):
        """Text tokenization workflow with session logging."""
        session_logger.info("Starting text tokenization (SOTA NLP)")
        cli = self._get_transcribe_cli(session_dirs)
        # Route tokenization output to the session tokenization directory
        cli.config.transcripts_dir = session_dirs["tokenization"]
        cli._tokenization_workflow()
        session_logger.info(f"Tokenization complete. Outputs in: {session_dirs['tokenization']}")

    def _dp_settings(self):
        """Data Preparation settings (Whisper model, device, cookies, HF token)."""
        self.logger.info("Opening Data Preparation settings")
        try:
            cli = self._get_transcribe_cli()
            cli._settings_menu()
            self.logger.info("Data Preparation settings closed")
        except KeyboardInterrupt:
            pass
        except Exception as e:
            self.logger.error(f"Settings error: {e}", exc_info=True)
            if HAS_RICH and self.console:
    ...
    def llm_annotation_studio(self):
        ...
        self._display_section_header(
            ...
            mode_info={
                ...
                'capabilities': ['2 LLM Providers (OpenAI/Ollama)', 'Prompt Wizard', '200K Context', 'Multi-Label Categories', 'NER', 'Pydantic Validation'],
...
- **Two LLM providers are supported**: OpenAI and Ollama (local). OpenAI needs an API key in environment variables. Ollama must be running before you start LLMTool.
...
4. Pick a model from **2 supported providers**: `openai:gpt-4o-mini` or local models like `ollama:llama3.2`. LLMTool validates that the provider is reachable.
