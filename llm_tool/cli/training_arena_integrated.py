    ...
    if format_choice == "llm-json":
        ...
        if True:  # Always ask
            ...
            if training_approach == "hybrid":
                ...
                stats_table = Table(show_header=True, header_style="bold magenta", box=box.ROUNDED, padding=(0, 1))
                stats_table.add_column("Key", style="yellow bold", width=20)
                stats_table.add_column("Value", style="white", width=25)
                stats_table.add_column("Instances", style="cyan", width=12, justify="right")
                stats_table.add_column("% of Key", style="green", width=10, justify="right")
                ...
                hybrid_sub_table = Table(show_header=True, header_style="bold", box=box.ROUNDED, padding=(0, 2))
                hybrid_sub_table.add_column("Option", style="bold cyan", width=20)
                hybrid_sub_table.add_column("Description", style="white")
...
def _run_benchmark_mode(
    ...
    if training_approach in ['hybrid', 'custom'] and onevsall_keys:
        ...
        if selected_onevsall:
            ...
            representative_selection = select_representative_benchmark_items(
                ...
                data=original_dataframe,
                annotation_column=annotation_column,
                max_multiclass=len(multiclass_keys),  # Keep all multi-class
            ...
            selection_table.add_column("#", style="yellow", width=3)
            selection_table.add_column("Item", style="cyan", width=45)
            selection_table.add_column("Strategy", style="green", width=12)
            selection_table.add_column("Type", style="blue", width=12)
            selection_table.add_column("Reason", style="white", width=50)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Determine which categories use multi-class vs one-vs-all
        # ============================================================
        training_approach = bundle.metadata.get('training_approach') if hasattr(bundle, 'metadata') else None
        multiclass_keys = bundle.metadata.get('multiclass_keys', []) if hasattr(bundle, 'metadata') else []
        onevsall_keys = bundle.metadata.get('onevsall_keys', []) if hasattr(bundle, 'metadata') else []
        ...
        if training_approach in ['hybrid', 'custom'] and (multiclass_keys or onevsall_keys):
