...
Contains ALL Training Arena code for complete integration.
"""
...
def integrate_training_arena_in_annotator_factory(
    ...
    """
    Integration for Annotator Factory - starts at STEP 3b with pre-selected columns.
    
    Uses ALL the Training Arena code above, but skips initial steps and starts
    directly at STEP 3b: Text Length Analysis.
    ...
    Args:
        ...
        session_dirs: Session directory structure
    ...
    console.print("\n" + "=" * 80)
    console.print("🎮 TRAINING ARENA - LLM Annotated Data Training")
    console.print("=" * 80 + "\n")
    console.print("[green]✓ Annotations loaded![/green]")
    ...
    analysis = detector.analyze_file(csv_path)
    all_columns = analysis.get('all_columns', [])
    ...
    # ========================================================================
    # START AT STEP 3b: Text Length Analysis
    # This is the EXACT code from Training Arena, starting at line 1777
    # ========================================================================
    
    # Step 3b: CRITICAL - Text Length Analysis
    console.print("\n[bold cyan]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[/bold cyan]")
    ...
    # Now continue with the complete Training Arena workflow by calling the wizard
    # We'll simulate being inside the llm-json section starting after column selection
    
    # Import what we need to continue
    from llm_tool.utils.language_detector import LanguageDetector
    from llm_tool.utils.language_normalizer import LanguageNormalizer
    
    # Step 5: Language Detection (EXACT code from Training Arena)
    console.print("\n[bold cyan]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[/bold cyan]")
    ...
    # Check if we have a language column
    has_lang_column = bool(analysis.get('language_column_candidates'))
    ...
    if has_lang_column and languages_found_in_column:
        ...
        if use_lang_column:
            ...
            console.print("\n[yellow]Language column not used. Applying automatic detection...[/yellow]")
            has_lang_column = False
    else:
        if not has_lang_column:
            console.print("[yellow]ℹ️  No language column detected[/yellow]")
        apply_auto_detection = Confirm.ask("Apply automatic language detection on text content?", default=True)
        if not apply_auto_detection:
            has_lang_column = True
    
    # Automatic language detection from text content
    if not lang_column and (not has_lang_column or 'apply_auto_detection' in locals()):
        console.print("\n[dim]🔍 Analyzing ALL texts to detect languages...[/dim]")
        ...
        try:
            if selected_text_column in df.columns:
                ...
                if all_texts:
                    detector_lang = LanguageDetector()
                    lang_counts = {}
                    ...
                    for text in tqdm(all_texts, desc="Detecting languages", disable=not HAS_RICH):
                        if text and len(str(text).strip()) > 10:
                            try:
                                detected = detector_lang.detect(str(text))
                                if detected:
                                    if isinstance(detected, dict):
                                        lang = detected.get('language')
                                        confidence = detected.get('confidence', 0)
                    ...
                    if lang_counts:
                        ...
                        # Add detected language column to DataFrame
                        if len(detected_languages_per_text) == len(df[df[selected_text_column].notna()]):
                            df['language'] = None
                            df.loc[df[selected_text_column].notna(), 'language'] = detected_languages_per_text
                            lang_column = 'language'
                            df.to_csv(csv_path, index=False)
                            console.print(f"[dim]✓ Added 'language' column to dataset[/dim]")
        except Exception as e:
            logger.debug(f"Language detection failed: {e}")
            console.print("[yellow]Automatic detection failed[/yellow]")
    
    # Now call the complete Training Arena dataset wizard with our prepared data
    # The wizard will handle all remaining steps (annotations, strategy, splits, etc.)
    bundle = cli_instance._training_studio_dataset_wizard(builder)
    ...
    if bundle:
        ...
        if text_length_stats:
            bundle.metadata['text_length_stats'] = text_length_stats
        bundle.metadata['source_file'] = str(csv_path)
        bundle.metadata['text_column'] = selected_text_column
    ...
    if bundle and session_dirs:
        ...
        console.print(f"\n[green]✓ Training metadata saved to session[/green]")
    
    # Show bundle summary (EXACT Training Arena)
    cli_instance._training_studio_render_bundle_summary(bundle)
    
    # Execute training (EXACT Training Arena)
    result = cli_instance._training_studio_confirm_and_execute(
        bundle=bundle,
        session_id=training_session_id
    )
