...
def safe_convert_label(label: Any) -> Union[int, str, List]:
    """
    Safely convert a single label from numpy types to Python native types.
    Also handles multi-label format (list of labels).

    Args:
        label: Label to convert (can be numpy.int64, int, str, list, etc.)

    Returns:
        Python native int, str, or list of labels (for multi-label format)
    ...
    Examples:
        ...
        "positive"
        >>> safe_convert_label(["label1", "label2"])
        ["label1", "label2"]
    """
    if isinstance(label, (np.integer, np.int64, np.int32, np.int16, np.int8)):
        ...
        # Multi-label format: recursively convert each element in the list
        return [safe_convert_label(item) for item in label]
    else:
        # Fallback: try to convert to string
        return str(label)


def safe_convert_labels(labels: List[Any]) -> List[Union[int, str, List]]:
    """
    Safely convert a list of labels from numpy types to Python native types.
    Also preserves multi-label format (list of labels per sample).

    Args:
        labels: List of labels to convert (can contain lists for multi-label)

    Returns:
        List of Python native ints, strs, or lists (for multi-label format)

    Examples:
        >>> safe_convert_labels([np.int64(1), np.int64(0), np.int64(1)])
        [1, 0, 1]
        >>> safe_convert_labels([['label1', 'label2'], ['label3']])
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            for row_num, row in enumerate(reader, 1):
                try:
                    ...
                    # CRITICAL FIX: Parse label if it's a string representation of a list
                    # (e.g., "['label1', 'label2']" from CSV storage)
