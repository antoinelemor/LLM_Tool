...
# Tail columns always present after per-class metrics
_TAIL_REQUIRED_COLS = [
    "macro_f1",
    "saved_model_path",
    "training_phase",
...
def _build_required_cols(best_row: Dict[str, str]) -> List[str]:
    """Build the list of required columns dynamically from the actual CSV row.

    Per-class columns now use class names (e.g., precision_civil_society)
    instead of numeric indices (precision_0), so we detect them dynamically.
    """
    cols = list(_BASE_REQUIRED_COLS)
    # Detect per-class columns from the actual row keys
    per_class_cols = [
        col for col in best_row.keys()
        if col.startswith(("precision_", "recall_", "f1_", "support_"))
        and not col.startswith(("f1_macro", "f1_micro"))
        and col not in ("f1_score",)
    ]
    cols.extend(per_class_cols)
    cols.extend(_TAIL_REQUIRED_COLS)
    return cols
...
class BenchmarkRunner:
    ...
    def _record_best_model(
        ...
        required_cols = _build_required_cols(best_row)
        ...
        for idx, (model_name, test_languages) in enumerate(models_to_test, 1):
            ...
            try:
                ...
                if needs_adjustment and self.config.large_model_adjustments:
                    ...
                    # NOTE: batch_size is optimized by system detection - DO NOT override
                    # Only adjust learning rate and epochs here
                    if is_short_seq:
    ...
    def _save_benchmark_logs(
        ...
        if save_detailed and results:
            ...
            headers = [
                ...
                'combined_score', 'f1_macro', 'f1_micro', 'accuracy',
            ...
            if is_multilingual:
                for lang in detected_languages:
                    headers.extend([
                        f'{lang}_f1_macro', f'{lang}_f1_micro', f'{lang}_accuracy',
            ...
            with open(detailed_file, 'w', newline='', encoding='utf-8') as f:
                ...
                for rank, result in enumerate(results, 1):
                    ...
                    # Extract metrics
                    f1_macro = result.get('f1_score', result.get('f1_macro', 0))
                    f1_micro = result.get('f1_micro', 0)
                    f1_0 = result.get('f1_class_0', 0)
                    f1_1 = result.get('f1_class_1', 0)

                    # Calculate combined_score using same formula as bert_base.py
                    # Binary: 0.7 × F1(class 1) + 0.3 × macro_f1
                    # Multi-class: 0.6 × macro_f1 + 0.4 × micro_f1
                    num_labels = result.get('num_labels', 2)
                    if num_labels == 2 and f1_1 > 0:
                        combined_score = 0.7 * f1_1 + 0.3 * f1_macro
                    elif f1_micro > 0:
                    ...
                    if is_multilingual and result.get('language_metrics'):
                        for lang in detected_languages:
                            ...
                            row[f'{lang}_f1_macro'] = lang_metrics.get('macro_f1', lang_metrics.get('f1_macro', 0))
                            row[f'{lang}_f1_micro'] = lang_metrics.get('micro_f1', lang_metrics.get('f1_micro', 0))
        ...
        if save_best and results:
            ...
            # Helper to calculate combined_score for a result
            def calc_combined_score(r):
                f1_macro = r.get('f1_score', r.get('f1_macro', 0))
                f1_micro = r.get('f1_micro', 0)
                f1_1 = r.get('f1_class_1', 0)
                num_labels = r.get('num_labels', 2)
                if num_labels == 2 and f1_1 > 0:
                    return 0.7 * f1_1 + 0.3 * f1_macro
                elif f1_micro > 0:
                    return 0.6 * f1_macro + 0.4 * f1_micro
                return f1_macro

            # Select best models by different criteria
            best_models = {
                'combined_score': max(results, key=calc_combined_score),
            ...
            with open(best_file, 'w', newline='', encoding='utf-8') as f:
                ...
                for criterion, model in best_models.items():
                    if model:
                        ...
                        reason = {
                            'combined_score': f"Highest combined score ({calc_combined_score(model):.3f})",
                            'overall_f1': f"Highest macro F1 score ({model.get('f1_score', 0):.3f})",
                            'minority_class': f"Best minority class detection (F1: {model.get('f1_class_1', 0):.3f})",
                            'fastest': f"Fastest inference ({model.get('inference_time', 0)*1000:.1f}ms)",
