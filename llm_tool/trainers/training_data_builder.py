...
class TrainingDatasetBuilder:
    ...
    def _build_llm_annotations(self, request: TrainingDataRequest) -> TrainingDataBundle:
        ...
        if request.mode == "single-label":
            ...
            if request.key_strategies and len(annotation_keys) > 1:
                ...
                for key in annotation_keys:
                    ...
                    elif strategy == 'multi-label':
                        multilabel_keys.append(key)
                    else:
                ...
                # Create files for multi-label keys (one file per key with sigmoid activation)
                for key in multilabel_keys:
                    key_output_file = dataset_dir / f"multilabel_{self._slugify(key)}_{suffix}.jsonl"
                    key_path = self.converter.create_multi_label_dataset(
                        csv_path=str(request.input_path),
                        output_path=str(key_output_file),
                        text_column=request.text_column,
                        annotation_column=request.annotation_column,
                        annotation_keys=[key],  # Only this key
                        label_strategy=request.label_strategy,
                        id_column=request.id_column,
                        lang_column=request.lang_column,
                    )
                    ...
                    if key_path:
                        category_files[key] = Path(key_path)
                        LOGGER.info(f"Created multi-label file for key '{key}': {key_output_file}")
                ...
                strategies_used = []
                if multiclass_keys:
                    strategies_used.append("multi-class")
                if multilabel_keys:
                    strategies_used.append("multi-label")
                if onevsall_keys:
                    strategies_used.append("one-vs-all")

                training_approach = "custom" if len(strategies_used) > 1 else (strategies_used[0] if strategies_used else "unknown")
            ...
            # Single key: create one file for multi-class training
            # CRITICAL FIX: Use create_single_key_dataset to produce proper multi-class format
            # (labels as scalar values, not lists), and populate training_files for consistency
            single_key = annotation_keys[0] if annotation_keys else "label"
            output_file = dataset_dir / f"multiclass_{self._slugify(single_key)}_{suffix}.jsonl"
            ...
            if not key_path:
                raise RuntimeError(f"Failed to create single-key dataset for key '{single_key}'")

            # Create training_files dict for consistency with multi-key case
            training_files = {single_key: Path(key_path)}
            ...
            metadata.update({
                ...
                "training_approach": "multi-class",
                "num_keys": 1,
                "files_per_key": {single_key: str(key_path)},
            })

            LOGGER.info(f"Created multi-class file for single key '{single_key}': {output_file}")

            # Return bundle with training_files populated for multi-class training flow
            return TrainingDataBundle(
                primary_file=Path(key_path),
                training_files=training_files,  # CRITICAL: Populate training_files!
                strategy="single-label",  # Single-label strategy for multi-class
                text_column="text",
                label_column="labels",  # create_single_key_dataset uses "labels" field (but with string value)
                metadata=metadata,
            )

        # Multi-label mode: create files based on number of keys
        # If multiple keys selected: create ONE multi-label file PER KEY (one model per key)
        # If single key: create one consolidated file (legacy behavior)

        if annotation_keys and len(annotation_keys) > 1:
            # MULTI-KEY MULTI-LABEL: Create one multi-label file per key
            # This allows training ONE multi-label model PER KEY instead of one giant model
            LOGGER.info(f"Multi-label mode with {len(annotation_keys)} keys - creating one file per key")

            category_files: Dict[str, Path] = {}

            for key in annotation_keys:
                # Create multi-label dataset for THIS KEY ONLY
                key_output_file = dataset_dir / f"multilabel_{self._slugify(key)}_{suffix}.jsonl"
                key_path = self.converter.create_multi_label_dataset(
                    csv_path=str(request.input_path),
                    output_path=str(key_output_file),
                    text_column=request.text_column,
                    annotation_column=request.annotation_column,
                    annotation_keys=[key],  # Only this key's labels
                    label_strategy=request.label_strategy,
                    id_column=request.id_column,
                    lang_column=request.lang_column,
                )
            ...
            if not category_files:
                raise RuntimeError("Failed to create any key-specific multi-label datasets")

            # Also create consolidated file for compatibility/reference
            consolidated_file = dataset_dir / f"multilabel_dataset_{suffix}.jsonl"
            self.converter.create_multi_label_dataset(
                csv_path=str(request.input_path),
            ...
            metadata.update({
                ...
                "files_per_key": {k: str(v) for k, v in category_files.items()},
                "multi_label_per_key": True,  # Flag to indicate per-key multi-label training
            })

            # Return bundle with training_files (one per key) for per-key training
            return TrainingDataBundle(
                primary_file=consolidated_file,
                training_files={**category_files, "multilabel": consolidated_file},
        ...
        # Single key or no keys: create ONE consolidated file (legacy behavior)
        ...
        metadata.update({
            "labels_detected": annotation_keys,
            "output_file": str(output_file),
            "multi_label_per_key": False,  # Single model for all labels
        })

        # Return bundle with primary_file but NO training_files
        # This will trigger single model MultiLabelTrainer path
        return TrainingDataBundle(
