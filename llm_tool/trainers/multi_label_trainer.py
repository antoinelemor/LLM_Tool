...
from llm_tool.trainers.training_logger import TrainingHyperparameters  # NEW: For rationalized logging
...
class TrainingConfig:
    ...
    multi_label_threshold: float = 0.5  # Threshold for multi-label predictions (labels with prob >= threshold are selected)
    # Device override for parallel training
    force_device: Optional[str] = None  # Force specific device ("cpu", "cuda", "mps") - used by parallel training workers
    # DataLoader optimization for parallel training - CRITICAL for GPU/CPU saturation
    dataloader_num_workers: int = 0  # Number of DataLoader workers (0 = main process, >0 = multiprocessing)
    ...
    rl_f1_threshold: float = 0.7  # Threshold to trigger reinforced learning
...
class MultiLabelTrainer:
    ...
    def train_single_model(self,
                         ...
                         batch_progress_callback: Optional[callable] = None,  # NEW: Batch-level progress
                         suppress_display: bool = False,
                         model_output_dir: Optional[str] = None,  # Override model output directory
                         training_approach: Optional[str] = None) -> ModelInfo:  # Override training approach for chart labeling
        """
        ...
        if use_enhanced:
            ...
            # CRITICAL FIX: Create unified label mapping from ALL labels (train + val)
            # This ensures consistent label indices across all data splits.
            # Without this, train and val would have different label-to-index mappings
            # if they have different label distributions, causing evaluation failures.
            # Handle both single-label (str/int) and multi-label (list) formats
            def flatten_labels(labels_list):
                """Extract unique labels from a list that may contain lists (multi-label) or scalars."""
                unique = set()
                for label in labels_list:
                    if isinstance(label, (list, tuple)):
                        # Multi-label: flatten the list
                        for l in label:
                            unique.add(l)
                    else:
                        # Single-label
            ...
            all_labels = list(flatten_labels(labels_train) | flatten_labels(labels_val))
            ...
            if self.verbose:
                logging.info(f"Created unified label mapping with {len(unified_label_mapping)} classes")

            train_loader = model.encode(texts_train, labels_train, batch_size=self.config.batch_size, progress_bar=False, shuffle=True, label_mapping=unified_label_mapping)
            val_loader = model.encode(texts_val, labels_val, batch_size=self.config.batch_size, progress_bar=False, shuffle=False, label_mapping=unified_label_mapping)
        ...
        # DEBUG: Log multi-label configuration for verification
        self.logger.info(f"[MULTI-LABEL CONFIG] multi_label={self.config.multi_label}, threshold={self.config.multi_label_threshold}, approach={self.config.training_approach}")

        # NEW: Create hyperparameters object for rationalized logging
        training_hyperparameters = TrainingHyperparameters(
            ...
            reinforced_epochs=self.config.n_epochs_reinforced,
        ...
        # Determine HuggingFace model name from model instance
        huggingface_model_name = getattr(model, 'model_name', None) or self.config.model_name
        ...
        scores = model.run_training(
            ...
            batch_progress_callback=batch_progress_callback,  # NEW: Batch-level progress for parallel dashboard
            # GPU Optimization parameters
            fp16=self.config.fp16,
            gradient_accumulation_steps=self.config.gradient_accumulation_steps,
            # Multi-label classification parameters
            ...
            # Parallel training: suppress display in worker processes
            suppress_display=suppress_display,
            # Parallel training: override model output directory (bypasses normal_training subfolder)
            model_output_dir=model_output_dir,
            # NEW: Rationalized logging parameters
        ...
        if training_summary:
            ...
            # CRITICAL: Add best_epoch if available
            if 'best_epoch' in training_summary:
                performance_metrics['best_epoch'] = training_summary['best_epoch']
        else:
        ...
        # Save detailed metrics - use session-aware path via resolve_metrics_base_dir
        metrics_base = str(resolve_metrics_base_dir(self.config.metrics_output_dir))
        metrics_dir = os.path.join(metrics_base, 'detailed_metrics')
    ...
    def train(self,
             ...
             rl_class_weight_factor: float = 2.0,
             progress_callback: Optional[callable] = None,
             training_approach: Optional[str] = None) -> Dict[str, ModelInfo]:
        """
        ...
        if training_approach is not None:
            # CRITICAL: Store training_approach for correct chart labeling
            self.config.training_approach = training_approach
    ...
    def _save_training_summary(self):
        ...
        metrics_base = str(resolve_metrics_base_dir(self.config.metrics_output_dir))
        summary_path = os.path.join(metrics_base, 'training_summary.json')
...
def train_models_parallel(
    ...
    """
    Train multiple models in parallel using GPU + CPU workers.

    This function leverages the ParallelTrainingOrchestrator to maximize
    resource utilization by training one model on GPU and multiple models
    on CPU cores simultaneously.

    Parameters
    ----------
    training_files : Dict[str, str]
        Dictionary mapping category names to their training data file paths.
        Example: {"sentiment": "data/sentiment.jsonl", "topic": "data/topic.jsonl"}
    model_name : str
        Name of the transformer model to use (e.g., "xlm-roberta-base")
    epochs : int
        Number of training epochs per model
    output_base : str
        Base directory for saving trained models
    max_cpu_workers : int, optional
        Maximum number of CPU training workers. If None, auto-detect based on
    ...
    -------
    Dict[str, Dict[str, Any]]
        Dictionary mapping category names to their training results:
        {
            "category_name": {
    ...
    results = train_models_parallel(
        training_files={
            ...
            "topic": "data/topic.jsonl",
            "intent": "data/intent.jsonl",
        },
    ...
    for category, result in results.items():
        if result["status"] == "success":
            print(f"{category}: F1={result['f1_score']:.4f}")
    ```
    """
    try:
        ...
        raise ImportError(
            "Parallel training requires the hybrid_parallel_trainer module. "
            f"Error: {e}"
        )

    # Create training tasks
    tasks = []
    for category_name, data_path in training_files.items():
        if category_name == "multilabel":
            continue  # Skip combined file

        task = TrainingTask(
            task_id=f"task_{category_name}",
            category_name=category_name,
            model_name=model_name,
            data_path=data_path,
            output_path=os.path.join(output_base, category_name),
            config={
                "epochs": epochs,
    ...
    if not tasks:
        logging.warning("No training tasks found")
        return {}

    # Create and run hybrid trainer
    trainer = HybridParallelTrainer(
        model_name=model_name,
        num_cpu_workers=max_cpu_workers if max_cpu_workers else 0,  # 0 = auto-calculate
    )

    # Run parallel training
    task_results = trainer.train(tasks=tasks)
    ...
    for result in results_list:
        category = result.get("category", result.get("task_id", "unknown"))
        results[category] = result

    return results


class ParallelMultiLabelTrainer:
    """
    Enhanced MultiLabelTrainer with parallel training support.

    This trainer extends the standard MultiLabelTrainer to support parallel
    training across GPU and CPU devices, maximizing resource utilization.

    Features
    --------
    - Automatic hardware detection and resource allocation
    - GPU + CPU parallel training
    - Unified Rich-based progress display
    - Dynamic batch size optimization per device
    - Graceful error handling and recovery

    Example
    -------
    ```python
    from llm_tool.trainers.multi_label_trainer import ParallelMultiLabelTrainer

    trainer = ParallelMultiLabelTrainer(
        model_name="xlm-roberta-base",
        epochs=10,
        output_dir="./models",
    )
    ...
    def __init__(
        ...
        multi_label : bool
            Whether to use multi-label classification
        max_cpu_workers : int, optional
            Max CPU workers (auto-detect if None)
        verbose : bool
    ...
    def analyze_resources(self) -> Dict[str, Any]:
        ...
        -------
        Dict
            Hardware analysis including GPU, CPU, and memory information
        """
        try:
            ...
            return {"error": "Hardware analyzer not available"}

    def create_resource_plan(self, total_tasks: int) -> Optional[Any]:
        """
        ...
        Returns
        -------
        ResourcePlan or None
            Resource allocation plan, or None if not available
        """
        try:
            from llm_tool.trainers.hybrid_parallel_trainer import HardwareAnalyzer
            analyzer = HardwareAnalyzer()
            return analyzer.create_resource_plan(
    ...
    def train_parallel(
        self,
        training_files: Dict[str, str],
        show_resource_plan: bool = True,
    ) -> Dict[str, Dict[str, Any]]:
        """
        Train models in parallel across GPU and CPU devices.
        ...
        show_resource_plan : bool
            Whether to display the resource allocation plan before training
        ...
        """
        if show_resource_plan and self.verbose:
            plan = self.create_resource_plan(len(training_files))
            if plan and self.console:
                self._display_resource_plan(plan)

        return train_models_parallel(
    ...
    def _display_resource_plan(self, plan):
        """Display the resource allocation plan."""
        try:
            ...
            self.logger.debug(f"Could not display resource plan: {e}")
